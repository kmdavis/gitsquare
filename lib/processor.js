...
var
  ...
  calculateMayorOf = function (repo, branch) {
    ...
    db.Commit.find(query, function (err, commits) {
      ...
      var commitsByAuthor = {}, nameByEmail = {}, authors = [];
      ...
      _.each(commits, function (commit) {
        if (!commitsByAuthor[commit.author.email]) {
          ...
          nameByEmail[commit.author.email] = commit.author.name;
        }
      ...
      db.Repository.update({ url: repo }, {
        rankings: _.map(commitsByAuthor, function (author) {
          return {
            author: {
              email: author.email,
              name: nameByEmail[author.email]
            },
            numberOfCommits: author.commits
          };
        })
      }, { upsert: true });
  ...
  processCommitsWorker = _.throttle(function () {
    ...
    repos = _.uniq(reposToProcess);
    reposToProcess = [];

    users = _.uniq(usersToProcess);
    usersToProcess = [];

    deferred.when(deferred.Deferred(function (dfd) {

      deferred.when.apply(null, _.map(repos, calculateMayorOf)).done(function () {
        dfd.resolve();
      });
      
    }, deferred.Deferred(function (dfd) {

      deferred.when.apply(null, _.map(users, calculateBadges)).done(function () {
        dfd.resolve();
      });
